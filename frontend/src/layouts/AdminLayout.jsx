...
import {
  FiGrid, FiUsers, FiCreditCard, FiShoppingBag, FiImage,
  FiMail, FiLogOut, FiMenu, FiX, FiBell, FiActivity, FiHome, FiTrendingUp
} from 'react-icons/fi';
...
export default function AdminLayout() {
  ...
  const navItems = [
    ...
    { name: 'Reports & Analytics', path: '/admin/reports', icon: FiTrendingUp }
  ];
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-gym-gray border-r border-white/5 py-6">
      ...
      <div className="px-6 mb-8 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full border border-red-500/30 bg-gym-card overflow-hidden flex-shrink-0 flex items-center justify-center">
          {user?.profilePicture ? (
        ...
        <div className="overflow-hidden">
          ...
          <span className="text-[10px] text-red-500 capitalize flex items-center gap-1 font-semibold">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-300 ${
                  isActive
                    ? 'bg-gradient-to-r from-red-600/10 to-red-600/0 border-l-2 border-red-500 text-red-500'
          ...
          <div className="flex items-center gap-4 ml-auto">
                    ...
                    >
                      <div className="flex justify-between items-center mb-3">
                        ...
                        {user?.notifications && user.notifications.length > 0 && (
                          <button
                            ...
                            className="text-[10px] text-red-500 hover:text-red-400 font-bold uppercase tracking-wider cursor-pointer transition-colors"
                          >
                            Clear All
                          </button>
                        )}
                      ...
                      <div className="flex flex-col gap-2">
                        {user?.notifications && user.notifications.length > 0 ? (
                          ...
                          <p className="text-xs text-gray-500 py-4 text-center">No system notifications yet.</p>
                        )}
                      </div>
                    </motion.div>
                  </>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                ...
                <span className="text-[10px] text-red-500 font-bold capitalize">System Admin</span>
              </div>
              <div className="w-10 h-10 rounded-full bg-red-500/10 border border-red-500/30 overflow-hidden flex items-center justify-center">
                {user?.profilePicture ? (
                  <img src={user.profilePicture} alt={user.name} className="w-full h-full object-cover" />
                ) : (
                  <span className="font-display text-sm font-bold text-red-500">
