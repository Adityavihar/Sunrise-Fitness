...
export default function LandingPage() {
  ...
  return (
    <div className="relative">
      ...
      {/* 5. TRAINERS SECTION */}
      <section id="trainers" className="py-24 bg-gym-gray relative">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-xl mx-auto mb-16">
            <span className="text-xs font-bold uppercase tracking-widest text-gold-500">Elite Trainers</span>
            <h2 className="text-3xl md:text-5xl font-black uppercase text-white tracking-tight mt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {defaultTrainers.map((trainer, idx) => (
              <motion.div
                key={idx}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: idx * 0.1 }}
                className="glass-panel overflow-hidden rounded-2xl group border border-white/5 hover:border-gold-500/20 transition-all duration-300"
              >
                <div className="aspect-[4/5] overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-gym-dark to-transparent opacity-80" />
                  <div className="absolute bottom-6 left-6 right-6">
                    <span className="text-[10px] uppercase tracking-widest text-gold-500 font-bold">{trainer.specialty}</span>
                    <h3 className="text-xl font-bold uppercase text-white tracking-wide mt-1 font-display">{trainer.name}</h3>
                    <p className="text-[10px] text-gray-400 mt-2 font-light">{trainer.cert}</p>
                  </div>
                </div>
              </motion.div>
            ))}
